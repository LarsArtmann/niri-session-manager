...
| 3 | v5 layout-geometry *application* in restore | Deliberately waits for the soak test | Yes, after (1) |
...
1. **Make CI enforce what locals enforce.** CI's testless clippy form was the *only*
...
| 2 | After push: fire `workflow_dispatch` again; assert green on the *fixed* head | High | S | Quality |
...
| 26 | Post-soak: design v5 geometry *application* (restore-side layout) | Medium | L | Feature |
...
*Point-in-time snapshot at 2026-09-15 18:16 CEST, code at local `15275fb` (+pending
...
truth. Section (f) is HARVEST input for TODO_LIST/ROADMAP.*
