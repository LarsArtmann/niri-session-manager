...
- **Real-hardware soak test** (`scripts/soak-test.sh`): a three-phase live-niri soak (A: reactive-save timing with burst/debounce/idle/open-close assertions; B: saved-session-vs-live-state fidelity; C: idempotent-restore proof with a real self-closing carrier window). First fully green run 2026-09-16 on niri unstable 2026-08-02: Phase A 19/19, Phase B all live windows matched, Phase C 13/13 (deficit restore spawns exactly the dead carrier, immediate re-restore spawns 0, boot gate holds, saved focus restored).
