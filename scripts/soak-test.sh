...
check_ge() {
  if [ "$2" -ge "$3" ]; then echo "PASS: $1 ($2 >= $3)"; else echo "FAIL: $1 (got '$2', want >= $3)"; FAILURES=$((FAILURES + 1)); fi
}
check_le() {
  if [ "$2" -le "$3" ]; then echo "PASS: $1 ($2 <= $3)"; else echo "FAIL: $1 (got '$2', want <= $3)"; FAILURES=$((FAILURES + 1)); fi
}
mark() { printf '%s %s\n' "$(date +%s.%N)" "$1" >> "$TIMELINE"; }
...
phase_a() {
  ...
  local S1; S1=$(saves_so_far)
  mark "focus-3";   focus 3; sleep 0.5
  mark "focus-2";   focus 2
  sleep 5
  check_le "focus round-trip collapsed to <=1 save" "$(( $(saves_so_far) - S1 ))" "1"

  local S2; S2=$(saves_so_far)
  mark "burst-start"
  local id
  for id in 3 2 3 2 3; do focus "$id"; sleep 0.3; done
  mark "burst-end"
  sleep 5
  check_le "toggle burst collapsed to <=1 save" "$(( $(saves_so_far) - S2 ))" "1"

  mark "focus-2-restore"; focus 2
  sleep 5

  local S3; S3=$(saves_so_far)
  mark "spawn-8s-1"; $NMSG msg action spawn -- ghostty -e sleep 8
  sleep 6; mark "after-open";  local S3_OPEN; S3_OPEN=$(saves_so_far)
  sleep 8; mark "after-close"; local S3_CLOSE; S3_CLOSE=$(saves_so_far)
  check_ge "window open produced a save" "$(( S3_OPEN - S3 ))" "1"
  check_ge "window close produced a save" "$(( S3_CLOSE - S3_OPEN ))" "1"

  local S4; S4=$(saves_so_far)
  mark "ws-roundtrip"; $NMSG msg action focus-workspace 3; sleep 1; $NMSG msg action focus-workspace 4
  sleep 5
  check_le "workspace round-trip <=1 save" "$(( $(saves_so_far) - S4 ))" "1"

  local S_IDLE; S_IDLE=$(saves_so_far)
  local MTIME_START; MTIME_START=$(stat -c %Y "$SESSION" 2>/dev/null || echo none)
  mark "idle-start"; sleep 50; mark "idle-end"
  check "idle: no saves" "$(saves_so_far)" "$S_IDLE"
  check "idle: mtime unchanged" "$(stat -c %Y "$SESSION" 2>/dev/null || echo none)" "$MTIME_START"

  local S5; S5=$(saves_so_far)
  mark "spawn-8s-2"; $NMSG msg action spawn -- ghostty -e sleep 8
  sleep 14
  check_ge "second open/close cycle saved" "$(( $(saves_so_far) - S5 ))" "1"

  mark "focus-3b"; focus 3; sleep 4
  mark "focus-2b"; focus 2; sleep 4

  # Carrier: alive at the final save, dead shortly after — Phase C's deficit.
  mark "spawn-45s-carrier"; $NMSG msg action spawn -- ghostty -e sleep 45
  sleep 6
