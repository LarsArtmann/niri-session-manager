...
- **This repo is a FORK of `MTeaHead/niri-session-manager`, and GitHub has never run its Actions**: fork workflows stay disabled until the owner clicks "enable" in the repo's Actions tab (no API consent exists — verified 2026-09-15: workflow `active`, triggers correct, 0 runs ever, while sibling repos run fine). Until enabled, treat CI gates as **local-parity only**: run the exact CI commands yourself before declaring anything green. After enabling, `workflow_dispatch` + `v*` tag triggers (added 2026-09-15) fire runs on demand and on release tags.
...
- 124 tests + 1 `#[ignore]`d benchmark. Unit tests live in `src/tests.rs`; IPC integration tests live in `src/fake_niri.rs` against the fake server.
- **CI asserts the passing test count** (`.github/workflows/checks.yml`, `expected=124`): adding tests is free, dropping tests fails CI. Bump `expected` only when intentionally removing tests. CI also runs a `--dry-run` smoke with a fresh `XDG_DATA_HOME` and no niri — it must exit 0 (restore failure is non-fatal); this is the guard that catches an accidentally fatal niri-absent path. (These gates have never actually executed on GitHub yet — see the fork/Actions warning in Commands.)
