...
| CI: assert the test count so a refactor cannot silently drop tests (the module split proved the risk), and record the ~16s suite timing budget | 🔴 `TODO` | Low    | Low    | suite is 120 tests (+1 ignored benchmark) as of 2026-09-15; the split needed a manual fn-name diff to prove zero test loss |
