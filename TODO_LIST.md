...
_(none — the real-hardware soak resolved 2026-09-16 together with the IPC-drift fix batch it discovered; see `CHANGELOG.md` [Unreleased] and `docs/status/` 2026-09-16 reports)_
...
_Verified 2026-09-16 against code at 131 passing tests (+1 ignored benchmark): the real-hardware soak item completed (Phase A 19/19, B all-match, C 13/13 — `scripts/soak-test.sh`), resolving together with the five IPC-drift/resilience/terminal-capture fixes it surfaced (see `CHANGELOG.md` [Unreleased]). The deployed 0.4.1 stays broken until SystemNix re-pins a release containing the fix._
